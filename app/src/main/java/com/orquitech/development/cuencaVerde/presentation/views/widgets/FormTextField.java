...
import com.orquitech.development.cuencaVerde.databinding.FormTextFieldBinding;
import com.orquitech.development.cuencaVerde.logic.Bus;
import com.orquitech.development.cuencaVerde.logic.RxBus;
import com.orquitech.development.cuencaVerde.presentation.views.viewModel.FormTextFieldMvvm;
...
public class FormTextField extends BaseAppView implements FormTextFieldMvvm.View {
    ...
    protected FormTextFieldBinding binding;
    private FormTextFieldMvvm.ViewModel viewModel;
    ...
    public FormTextField(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void init(Context context) {
        if (!isInEditMode()) {
            ...
            binding = DataBindingUtil.inflate(inflater, R.layout.form_text_field, this, true);
            viewModel = (FormTextFieldMvvm.ViewModel) viewModelFactory.getViewModel(this, AppViewModelsFactory.FORM_TEXT_FIELD_VIEW_MODEL);
    ...
    private void showIsMandatoryAlert() {
        boolean isEmailAndIsNotValid = false;
        if (fieldType.equals(FIELD_TYPE_EMAIL)) {
            isEmailAndIsNotValid = !FormUtils.isEmailValid(binding.textField.getText().toString());
        }
        if (isMandatory && (TextUtils.isEmpty(binding.textField.getText()) || isEmailAndIsNotValid)) {
    ...
    private void setUpFieldType() {
        Log.d("inputType", fieldType);
        switch (fieldType) {
            case FIELD_TYPE_NUMBER:
                binding.textField.setInputType(InputType.TYPE_CLASS_NUMBER);
                break;
